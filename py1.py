			...
			try:
				with open("authToken.txt", "r") as token:
					...
					if not authToken:
						...
						continue
					client = LINE(authToken, speedThrift=False, appName="{}\t2.1.5\tHelloWorld\t2.1.5".format(app))
				break
...
def sendMentionFooter(to, text="", mids=[]):
    ...
    client.sendMessage(to, textx, {'AGENT_NAME':'LINE MUSIC','AGENT_ICON': "http://dl.profile.line-cdn.net/" + client.getProfile().picturePath, 'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)    
...
def menuTextToSpeech1():
	if settings['setKey'] == True:
		key = settings['keyCommand']
	else:
		key = ''
	menuTextToSpeech1 ="LINE HIBURAN"
	return menuTextToSpeech1
...
#								ret_ +="\n╠ Auto Respon Message : {}".format(settings["autoResponMessage"])
							...
							helpTextToSpeech1 = menuTextToSpeech1()
							contact = client.getContact(sender)
							icon = "http://dl.profile.line-cdn.net/{}".format(contact.pictureStatus)
							name = contact.displayName
							link = "https://www.google.com"
							client.sendFooter(to, helpTextToSpeech1, icon, name, link)
#							client.sendMention(to, "@!\nLINE", [sender])
							...
							midSelect = len(midMembers)//1000
							for mentionMembers in range(midSelect+1):
								no = 0
								ret_ = "╔════════════════\n╠  Mention Members \n╠════════════════\n╠════════════════"
								dataMid = []
								for dataMention in group.members[mentionMembers*1000 : (mentionMembers+1)*100]:
